...
- `/stop` 指令的行为：
  - 如果当前 chat 有正在进行的 AI 思考，则取消该任务（终止 OpenCode 子进程），回复"已中断当前 AI 思考。"
  - 如果当前 chat 没有正在进行的 AI 思考，则回复"当前没有进行中的 AI 思考。"
...
当 AI 思考时间较长时，nochan 会自动向用户发送通知：

| 阈值 | 通知内容 |
|---|---|
| `thinking_notify_seconds`（默认 10 秒） | "消息已收到，AI 正在思考中，请稍候..." |
| `thinking_long_notify_seconds`（默认 30 秒） | "AI 思考时间较长，你可以发送 /stop 中断当前思考。" |

两个阈值均可在 `config.toml` 的 `[ux]` 段中配置，设为 0 可禁用对应通知。

### 7.2 并发消息处理

当某个 chat 已有正在进行的 AI 思考时，新的普通消息会被拒绝并提示用户：

> AI 正在思考中，请等待或使用 /stop 中断。

指令（`/new`、`/stop`、`/help`）不受此限制，始终可以立即处理。不同 chat 之间互不影响。
...
5. 消息转换：OneBot 消息段 → 纯文本（附加来源上下文）
6. 检查是否为指令：
   - /new → 归档当前会话，创建新会话，回复"已创建新会话"
   - /stop → 取消当前 chat 的活跃 AI 任务（如有），回复结果
   - /help → 回复帮助信息
   - /其他 → 回复帮助信息（无效指令）
   - 普通消息 → 进入步骤 7
7. 检查该 chat 是否有正在进行的 AI 思考
   - 有 → 拒绝并提示"AI 正在思考中，请等待或使用 /stop 中断"
   - 无 → 继续
8. 查询该 chat_id 的激活会话
   - 无激活会话 → 自动创建新会话
   - 有激活会话 → 使用该会话
9. 启动超时通知定时器（thinking_notify / thinking_long 阈值）
10. 将消息提交到 OpenCode 封装层的请求队列
    - 如需等待 → 向用户发送"正在排队..."提示
11. OpenCode 封装层调用 opencode run，获取 AI 回复
    - 如被 /stop 取消 → 终止 opencode 子进程，清理资源
12. 取消所有未触发的超时通知定时器
13. 消息转换：AI 回复文本 → OneBot 消息段
14. 通过 WebSocket 调用 send_msg API 发送回复
```
...
| 用户发送 /stop 中断 AI 思考 | 终止 opencode 子进程，向用户发送"已中断当前 AI 思考。" |
| AI 思考期间收到新的普通消息 | 拒绝并提示"AI 正在思考中，请等待或使用 /stop 中断。" |
...
[ux]
thinking_notify_seconds = 10     # 首次"AI 正在思考"通知的等待秒数（0 = 禁用）
thinking_long_notify_seconds = 30 # 第二次"思考时间较长"通知的等待秒数（0 = 禁用）
...
- **自动超时**：为 OpenCode 调用设置自动超时上限（如 5 分钟），超时后自动终止进程并通知用户
